...
def spea2(pop_size,
    ...
    return SPEA2(pop_size=pop_size, ref_dirs=ref_dirs, sampling=sampling, crossover=crossover,
        mutation=mutation, eliminate_duplicates=True)
...
def nsga_2(pop_size,
    ...
    return NSGA_2(pop_size=pop_size, sampling=sampling, crossover=crossover,
        mutation=mutation, eliminate_duplicates=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--moea', type=str, default='nsga_2', help='which MOEA to run')
    ...
    # for pid in range(6,7):
        for r in range(1, args.runs+1):
            ...
            #     algorithm = spea2(pop_size,ref_dirs)
            # elif args.moea == 'nsga_2':
            #     algorithm = nsga_2(pop_size)
            # else:
            #     raise NotImplementedError
          ...
          mut_eta=20.0,  # polynomial mutation hyperparameter eta
            ...
            algorithm.set_up(problem,benchmark)
            res = algorithm.run(n_gen)
            ...
            # run_stats['F'] = F
            # print(res)
            hv = benchmark.calc_perf_indicator(res, 'hv')
            print(hv)
            # print(len(res['X']))
        #     run_stats['X'] = res.X
        #     if pid < 8:
